...
// prepare proxy config and replace config
var packageJson = JSON.parse(fs.readFileSync('./package.json', 'utf8'));
var proxyConfig = [];
var replaceConfig = {};
if (packageJson && packageJson['projectConfig']) {
    if (packageJson['projectConfig']['proxies']) {
        proxyConfig = packageJson['projectConfig']['proxies'];
    }
    if (packageJson['projectConfig']['replaceConfig']) {
        var target = "dev";
        if (process.env.TARGET) {
            target = process.env.TARGET;
        }
        replaceConfig = packageJson['projectConfig']['replaceConfig'][target];
    }
}
...
gulp.task('styles', function() {
    ...
    var options = build ? {
     ...
     // build animate.css dynamically to support custom themes
     // build jquery-ui dynamically to support custom themes
     var pluginStream = (build
         ? gulp.src(['bower_components/jquery-ui/themes/base/jquery-ui.min.css'])
         : gulp.src(['bower_components/jquery-ui/themes/base/jquery-ui.css']))
    ...
    return streamqueue({
            objectMode: true
        }, pluginStream, ionicStream, sassStream)
...
gulp.task('scripts', function() {
    ...
    var scriptStream = gulp
        .src(['templates.js', 'app.js', 'config.js', '**/*.js'], {
            cwd: 'app/scripts'
        })
    ...
    if (build) {
        Object.keys(replaceConfig).forEach(function(key, index) {
            var s = '/*REPLACE*/' + key + '/*REPLACE*/',
                r = '/*REPLACE*/' + replaceConfig[key] + '/*REPLACE*/';

            console.log('replace "' + s + '" as "' + r + '"');
            scriptStream.pipe(plugins.replace(s, r, {
                skipBinary: true
            }));
        });
    }
...
gulp.task('images', function() {
    return gulp.src(['bower_components/jquery-ui/themes/base/images/**/*.*', 'app/images/**/*.*'])
...
// handle the html files except index.html
gulp.task('otherhtml', function() {
    // copy html except index.html
...
gulp.task('serve', function() {
    var exp = express();
    exp.use(!build ? connectLr() : function() {});
    exp.use(express.static(targetDir));

    proxyConfig.forEach(function(obj, idx) {
        console.log(obj['path']);
        exp.use(obj['path'], proxy({
            target: obj['proxyUrl'],
            changeOrigin: true,
            logLevel: 'debug'
        }))
...
gulp.task('watchers', function() {
    ...
    gulp.watch(['app/*.html', '!app/index.html'], ['otherhtml']);
